...
router.get('/weakPron', async function (req, res, next) {
    try {
        ...
        documents.forEach(doc => {
            for (const phoneme in doc.phonemes) {
                const weakPercentage = doc.phonemes[phoneme].sum / doc.phonemes[phoneme].count;
                const ratioPercentage = Math.floor(weakPercentage);
                if (ratioPercentage < 60) {
                    weakPhonemes.push({ phoneme, weakPercentage: ratioPercentage });
                }
            }
        });

        weakPhonemes.sort((a, b) => a.weakPercentage - b.weakPercentage);

        if (weakPhonemes.length > 4) {
            weakPhonemes = weakPhonemes.slice(0, 4);
        }

        res.status(200).json(weakPhonemes);
    } catch (error) {
        res.status(500).json({ error: '서버 내부 문제로 인해 요청을 처리할 수 없습니다.' });
    }
});
